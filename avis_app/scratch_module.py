...
def load_drive():
    ...
    creds_dict = {
        ...
        "private_key": st.secrets["GDRIVE_SERVICE_ACCOUNT"]["private_key"],
    ...
    if uploaded_file:
        ...
        if st.button("Run Detection"):
            ...
            if masks:
                ...
                save_data_to_csv_drive(filename=f"overlay_{now}.jpg", severity=severity, confidence=confidence,
                                       pixels=total_pixels, folder_id="1QIIdYHFt-iWAd-jvE1KAESEzBjWkb3gv")

            else:
                st.warning("No scratches detected.")
                ist = timezone('Asia/Kolkata')
                now = datetime.now(ist).strftime('%Y%m%d_%H%M%S')
                save_data_to_csv_drive(filename=f"overlay_{now}.jpg", severity="none", confidence="none", pixels="none",
                                       folder_id="1QIIdYHFt-iWAd-jvE1KAESEzBjWkb3gv")
